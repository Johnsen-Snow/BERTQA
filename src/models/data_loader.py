...
class SQuADDataLoader(BaseDataLoader):
    ...
    def preprocessing(self, filepath, is_training=True):
        ...
        examples = []
        for entry in tqdm(input_data, ncols=80, desc='traversing each paragraph'):
                    ...
                    if is_training:
                        ...
                        if not is_impossible:
                            ...
                            if actual_text.find(cleaned_answer_text) == -1:
                                logger.info(f'Could not find answer: {actual_text} vs. {cleaned_answer_text}')
                                continue
                        else:
                            start_position = -1
                            end_position = -1
    ...
    def data_process(self, filepath, is_training=False):
        ...
        if os.path.exists(data_path):
            with open(data_path, 'rb') as f:
                logger.info(f'loading processed data {data_path}')
                data = torch.load(f)
            return data

        logger.info(f'processing data {filepath}')
        examples = self.preprocessing(filepath, is_training)
        all_data = []
        example_id, feature_id = 0, 1000000000

        for example in tqdm(examples, ncols=80, desc='traversing each example'):
    ...
    def write_prediction(self, test_iter, all_examples, logits_data, output_dir):
        ...
        with open(os.path.join(output_dir, f'best_result_v2.0.json'), 'w') as f:
